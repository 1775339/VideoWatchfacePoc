...
class VideoExtractFrameAsyncHelper() {
    ...
    fun getVideoThumbnailsOfTimmedVideo(
        videoPath: String?,
        outPutFileDirPath: String,
        startPosition: Long,
        endPosition: Long,
        thumbnailsCount: Int,
    ): ArrayList<VideoThumbImg> {
        var thumbNailList = ArrayList<VideoThumbImg>()
        val metadataRetriever = MediaMetadataRetriever()
        metadataRetriever.setDataSource(videoPath)
        val interval = (endPosition - startPosition) / (thumbnailsCount - 1)
        for (i in 0 until thumbnailsCount) {
            ...
            if (i == thumbnailsCount - 1) {
                if (interval > 1000) {
                    ...
                    path?.let {
                        thumbNailList.add(VideoThumbImg(it, endPosition - 800))
                    }
                } else {
                    val path = extractFrame(metadataRetriever, endPosition, outPutFileDirPath)
                    path?.let {
                        thumbNailList.add(VideoThumbImg(it, endPosition))
//                        callback(VideoThumbImg(it, endPosition))
                    }
                }
            } else {
                val path = extractFrame(metadataRetriever, time, outPutFileDirPath)
                path?.let {
                    thumbNailList.add(VideoThumbImg(it, time))
//                    callback(VideoThumbImg(it, time))
                }
            }
        }
